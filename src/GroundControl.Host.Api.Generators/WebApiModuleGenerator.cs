...
public class WebApiModuleGenerator : IIncrementalGenerator
{
    ...
    private static Dictionary<string, string> BuildVariableNameMap(ImmutableArray<ModuleInfo> modules)
    {
        ...
        foreach (var module in modules)
        {
            ...
            {
                if (!nameIndex.ContainsKey(baseName))
                {
                    nameIndex[baseName] = 1;
                }

                result[module.FullyQualifiedName] = baseName + nameIndex[baseName];
                nameIndex[baseName]++;
            }
            else
            {
