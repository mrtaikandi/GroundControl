...
internal sealed class UpdateProjectValidator : IAsyncValidator<UpdateProjectRequest>
{
    private readonly IGroupStore _groupStore;
    private readonly ITemplateStore _templateStore;

    public UpdateProjectValidator(IGroupStore groupStore, ITemplateStore templateStore)
    {
        _groupStore = groupStore ?? throw new ArgumentNullException(nameof(groupStore));
        _templateStore = templateStore ?? throw new ArgumentNullException(nameof(templateStore));
    }

    public async Task<ValidatorResult> ValidateAsync(UpdateProjectRequest instance, ValidationContext context, CancellationToken cancellationToken = default)
    {
