...
export function ProjectTabs({ clientCount, configCount, projectId, snapshotCount }: ProjectTabsProps) {
  ...
  return (
    <nav aria-label="Project sections" className="flex flex-wrap gap-1 border-b border-stroke-divider">
      {tabs.map((tab) => {
        const Icon = tab.icon;
        ...
        return (
          <Link
            className={cn(
              '-mb-px inline-flex items-center gap-2 border-b-2 px-3 py-2.5 text-[13px] transition-colors',
