    ...
    public static global::Microsoft.AspNetCore.Builder.WebApplication BuildWebApiModules(this global::Microsoft.AspNetCore.Builder.WebApplicationBuilder builder)
    {
        ...
        if (IsModuleEnabled(builder.Configuration, "App"))
        {
            var appModuleOptions = BindOptions<global::AppOptions>(
                ...
                {
                    var maxRetriesPrimarySection = configuration.GetSection("App:MaxRetries");
                    if (!maxRetriesPrimarySection.Exists())
                    {
                        var maxRetriesFallbackSection = configuration.GetSection("Limits:MaxRetries");
                        if (maxRetriesFallbackSection.Exists())
                        {
                            options.MaxRetries = maxRetriesFallbackSection.Get<int>()!;
                        }
                    }
                });
